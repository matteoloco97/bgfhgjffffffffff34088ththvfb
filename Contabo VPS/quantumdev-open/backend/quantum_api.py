...
BUILD_SIGNATURE = (
    ...
    "search-diversifier+synthesis-aggressive-v2+llm-intent+jarvis-uncensored-v1+adaptive-timeout+snippet-synthesis"
)
...
# ⚡️ Parallel fetch env (PATCHED VALUES)
WEB_FETCH_TIMEOUT_S = env_float("WEB_FETCH_TIMEOUT_S", 5.0)      # ↑ da 3.0
WEB_FETCH_MAX_INFLIGHT = env_int("WEB_FETCH_MAX_INFLIGHT", 6)    # ↑ da 4
WEB_READ_TIMEOUT_S = env_float("WEB_READ_TIMEOUT_S", 8.0)        # ↑ da 6.0
...
# ====== WEB SYNTHESIS PROMPT V2 (PATCH) ===============================

SYNTHESIS_PROMPT_V2 = """
TU SEI UN ESTRATTORE DI VALORE ESPERTO.

MISSIONE: Estrarre OGNI informazione utile dagli estratti e presentarla in modo CONCRETO.
...
🚫 VIETATO (fallimento immediato):
- "Le fonti non contengono"
- "Non ho abbastanza informazioni"
- "Ti consiglio di aprire/consultare le fonti"
- "Per maggiori dettagli..."
- Qualsiasi frase che rimanda l'utente altrove

✅ OBBLIGATORIO:
1. **Estrai TUTTI i fatti concreti**: numeri, date, nomi, prezzi, risultati, quote
2. **Forma narrativa**: 4-8 frasi fluide, NO bullet points
3. **Minimo 3 fatti verificabili** nella risposta
4. **Se info parziali**: "Dagli estratti emergono [A, B, C], ma mancano dettagli su [X]"
5. **Se off-topic**: "Le fonti parlano di [tema A] menzionando [dati concreti], ma non coprono [tema richiesto]. Per quello serve cercare [suggerimento specifico con keywords]"

=== STRATEGIA DI ESTRAZIONE ===

STEP 1: Identifica TUTTI i fatti (anche se non direttamente correlati)
STEP 2: Organizza per rilevanza alla domanda
STEP 3: Presenta i 3-5 fatti più utili in forma narrativa
STEP 4: Se mancano info, specifica esattamente COSA manca (non dire genericamente "non c'è abbastanza")

ESEMPI DI BUONE RISPOSTE:

Query: "Prezzo Bitcoin oggi"
❌ BAD: "Le fonti non forniscono il prezzo esatto di Bitcoin."
✅ GOOD: "Secondo i dati raccolti, Bitcoin quota attorno ai $43,250 con volumi di trading elevati nelle ultime 24h. Le fonti evidenziano un rialzo del 3.2% rispetto a ieri, principalmente guidato da acquisti istituzionali. Non sono specificati i prezzi su exchange specifici italiani."

Query: "Chi ha vinto Juventus-Milan?"
❌ BAD: "Ti consiglio di consultare le fonti per i risultati."
✅ GOOD: "Juventus ha battuto Milan 2-1 con gol di Vlahovic (35') e Chiesa (67'), mentre per il Milan ha segnato Leao (42'). La partita si è giocata ieri sera all'Allianz Stadium davanti a 40.000 spettatori."

ORA RISPONDI SEGUENDO TUTTE LE REGOLE:
"""
...
# ======= TIMEOUT ADATTIVO PER WEB FETCH (PATCH) =======================

_DOMAIN_LATENCIES: Dict[str, Tuple[float, int]] = {}


def _update_domain_latency(url: str, dt: float) -> None:
    """Aggiorna la latenza media osservata per il dominio della URL."""
    try:
        host = urlparse(url).netloc.split(":")[0]
        if not host:
            return
        total, count = _DOMAIN_LATENCIES.get(host, (0.0, 0))
        total += float(dt)
        count += 1
        _DOMAIN_LATENCIES[host] = (total, count)
    except Exception:
        return


def _get_domain_avg_latency(url: str) -> float:
    """Ritorna la latenza media osservata (in secondi) per il dominio."""
    try:
        host = urlparse(url).netloc.split(":")[0]
        if not host:
            return 0.0
...
def calculate_adaptive_timeout(url: str, avg_latency: float) -> float:
    """
    Calcola timeout adattivo basato su:
    - Dominio (alcuni sono notoriamente lenti)
    - Latenza media storica
    """
    ...
    base_timeout = slow_domains.get(domain, WEB_FETCH_TIMEOUT_S or 5.0)

    # Adjust based on historical avg
    if avg_latency > 3.0:
        base_timeout = min(10.0, base_timeout * 1.5)
...
# ===================== Snippet synthesis helper (PATCH) ===============


async def synthesize_from_snippets(
    query: str,
    serp_results: List[Dict],
    persona: str,
) -> str:
    """
    Se non abbiamo estratti completi, sintetizza dagli snippet SERP.
    Usa un prompt specializzato per snippet corti.
    """
    snippet_ctx = "\n\n".join(
        [
    ...
    prompt = f"""
CONTESTO: Hai solo snippet brevi dai risultati di ricerca (non pagine complete).
...
COMPITO: Anche con snippet limitati, SINTETIZZA ciò che emerge:

1. **Identifica il pattern comune** tra gli snippet
2. **Estrai tutti i fatti menzionati** (anche parziali)
3. **Fornisci una sintesi utile** (3-5 frasi)
4. **Se info incomplete**: specifica ESATTAMENTE cosa serve cercare per dettagli completi

ESEMPIO:
Query: "Prezzo azioni Tesla oggi"
Snippet 1: "Tesla shares rose 3% in pre-market..."
Snippet 2: "TSLA stock price $245.30..."
Snippet 3: "Tesla gains on strong earnings..."

✅ RISPOSTA: "Le azioni Tesla mostrano un trend positivo con un prezzo attorno ai $245 e un rialzo del 3% nel pre-market. I guadagni sono guidati da risultati trimestrali sopra le attese. Per quotazioni in tempo reale specifiche serve controllare i dati live di borsa."
...
async def _web_search_pipeline(
    ...
    # Persona usata per la sintesi (estratti o snippet)
    ...
    # === ⚡ PARALLEL FETCHING con SMART BUDGET + TIMEOUT ADATTIVO ⚡ ===
    ...
    if topk and nsum > 0:
        total_budget = WEB_SUMMARY_BUDGET_TOK
        ...
        if nsum == 1:
            ...
            ] + [int(total_budget * 0.1 / (nsum - 3))] * (nsum - 3)

        # rileva se fetch_and_extract è async o sync
        import inspect

        _FETCH_IS_ASYNC = inspect.iscoroutinefunction(fetch_and_extract)
        ...
        async def _fetch_one(
            ...
            avg_lat = _get_domain_avg_latency(url)
            timeout = calculate_adaptive_timeout(url, avg_lat)

            try:
                t0_local = time.perf_counter()
                if _FETCH_IS_ASYNC:
                    # caso: fetch_and_extract è coroutine
                    text, og_img = await asyncio.wait_for(
                        fetch_and_extract(url, timeout=timeout),  # type: ignore[arg-type]
                        timeout=timeout + 1.0,
                    )
                else:
                    # caso: fetch_and_extract è funzione sincrona → eseguila in thread
                    text, og_img = await asyncio.wait_for(
                        asyncio.to_thread(fetch_and_extract, url, timeout=timeout),
                        timeout=timeout + 1.0,
                    )
                dt = time.perf_counter() - t0_local
                _update_domain_latency(url, dt)
                ...
                return {
                    ...
                    "text": trimmed,
                    "og_image": og_img,
                    "budget_used": approx_tokens(trimmed),
                    "index": index,
                }
            except asyncio.TimeoutError:
                timeouts += 1
                log.warning(f"Fetch timeout ({timeout}s): {url}")
                return None
            except Exception as e:
                errors += 1
        ...
        tasks = [
            asyncio.create_task(_fetch_one(r, budgets[i], i))
            for i, r in enumerate(topk[:nsum])
        ]
        attempted = len(tasks)

        early_exit_time = 3.0
        early_exit_min = min(2, nsum)

        try:
            done, pending = await asyncio.wait(
                tasks,
                timeout=early_exit_time,
                return_when=asyncio.FIRST_COMPLETED if nsum == 1 else asyncio.ALL_COMPLETED,
            )

            results_so_far = [
                t.result()
                for t in done
                if not t.exception() and t.result() is not None  # type: ignore[union-attr]
            ]

            if len(results_so_far) >= early_exit_min and pending:
                log.info(
                    f"Early exit: {len(results_so_far)}/{nsum} results in "
                    f"{time.perf_counter() - t_fetch_start:.1f}s"
                )
                for t in pending:
                    t.cancel()
                done_early = True
                extracts = [e for e in results_so_far if e is not None]
            else:
                if pending:
        ...
        extracts.sort(key=lambda x: x.get("index", 999))  # type: ignore[arg-type]
    ...
    if extracts:
        ...
        prompt = SYNTHESIS_PROMPT_V2.format(context=ctx, query=q)
        ...
        if summary:
            try:
                ...
                if syn_validation["score"] < 0.5 and (
                    ...
                    try:
                        summary_retry = await reply_with_llm(retry_prompt, persona)
                        if summary_retry:
                            syn_validation_retry = validator.validate(summary_retry)
                            if (
        ...
        # PATCH: se non abbiamo estratti ma abbiamo snippet SERP, sintetizza dagli snippet
        if topk:
            try:
                summary = await synthesize_from_snippets(q, topk, persona or DEFAULT_SYSTEM_PROMPT)
            except Exception as e:
                log.warning(f"Snippet synthesis failed: {e}")
                summary = summary or ""
...
async def chat(payload: dict = Body(...)) -> Dict[str, Any]:
      ...
      * payload nuovo stile:
        {
          "messages": [
            {"role": "user", "content": "..."},
            {"role": "system", "content": "..."}  // opzionali, multipli
          ],
          "source": "gui|tg|api" (opzionale),
          "source_id": "..." (opzionale),
          "system_prompt": "..." (opzionale)
        }
    ...
    - Usa Chroma (search_topk) per facts
    - Usa semantic cache dedicata a CHAT
    - Caso speciale hardware Jarvis/AI personale: zero hallucination su CPU/GPU
    """
    global _SEMCACHE
    ...
    if _SEMCACHE:
        ...
        if hit:
            ...
            if sim >= 0.88:
                ...
                try:
                    asv_out = autosave(reply_cached, source="chat_reply_cache")
                    if any(
                        [
                            asv_out.get("facts"),
                            asv_out.get("prefs"),
                    ...
                    log.warning(f"AutoSave chat_reply_cache failed: {e}")
    ...
    for it in mem_items:
        ...
        subj = (md.get("subject") or md.get("key") or md.get("label") or "").lower()
...
async def web_search(req: WebSearchReq) -> Dict[str, Any]:
    ...
    ws = await _web_search_pipeline(
        ...
        k=int(req.k or 6),
        nsum=int(req.summarize_top or WEB_SUMMARIZE_TOP_DEFAULT),
    )
    out: Dict[str, Any] = {
