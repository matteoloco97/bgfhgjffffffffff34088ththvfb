...
BUILD_SIGNATURE = (
    ...
    "search-diversifier+synthesis-aggressive-v1+llm-intent+jarvis-uncensored-v1"
)
...
async def _web_search_pipeline(
    ...
    # Nessun risultato: eventuale fallback statico (meteo/prezzi)
    if not dedup:
        fallback = _safe_fallback_links(q)
        if not fallback:
            return {
                "query": q,
                "policy_used": pol,
        ...
        # Se abbiamo fallback statico, usalo come dedup
        dedup = [
            {
                "url": item["url"],
                "title": item.get("title", item["url"]),
                "snippet": item.get("title", item["url"]),
            }
    ...
    # ⭐ Fallback SNIPPET: se non abbiamo estratti dalle pagine, usa gli snippet SERP
    if not extracts and topk:
        snippet_extracts: List[Dict[str, Any]] = []
        for idx, item in enumerate(topk[:nsum]):
            snippet = (item.get("snippet") or "").strip()
            if not snippet:
                continue
            trimmed = trim_to_tokens(snippet, WEB_EXTRACT_PER_DOC_TOK)
            snippet_extracts.append(
                {
                    "url": item.get("url", ""),
        ...
        if snippet_extracts:
            log.info(
                f"Using SERP snippet fallback: {len(snippet_extracts)} extracts (no page content)"
            )
            extracts = snippet_extracts
