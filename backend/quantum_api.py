...
BUILD_SIGNATURE = "smart-intent-2025-11-10+env-safe+lazy-semcache+token-budget+summarize-q+no-error-user+feedback-toggle+web-search-endpoint+meta-override+zero-web-guard+no-generic-fallback"
...
def _is_smalltalk_query(q: str) -> bool:
    s = (q or "").strip().lower()
    return bool(_SMALLTALK_RE.search(s)) or len(s.split()) <= 2
...
async def _web_search_pipeline(q:str, src:str, sid:str, k:int, nsum:int)->Dict:
    # 🔒 Guard: non cercare MAI su smalltalk/very-short
    ...
    # Estrai testo ma con HARD CAP per documento
    extracts=[]
    for r in topk[:max(0,nsum)]:
        try:
            ...
            if text:
                ...
                extracts.append({"url":r["url"],"title":r["title"],"text":trimmed})
        except Exception: pass

    # Query "live" → niente LLM, risposta breve + fonti
    summary=""
    if _is_quick_live_query(q):
        summary = "Dato live: apri la fonte per il valore aggiornato."
    else:
        # LLM con contesto limitato
        if extracts:
            persona=await get_persona(src, sid)
            ctx="\n\n".join([f"### {e['title']}\nURL: {e['url']}\n\n{e['text']}" for e in extracts])
            ctx = trim_to_tokens(ctx, WEB_SUMMARY_BUDGET_TOK)
            prompt=(
                "Leggi gli estratti e rispondi in 3–5 frasi, in ITALIANO, molto concreto.\n"
                "- NON inventare numeri: usa solo quelli presenti.\n"
                "- Se mancano dati live, invita ad aprire la fonte.\n"
                "- Concludi con una riga: Fonti: <URL1>[, <URL2>]\n\n"
                f"QUERY: {q}\n\n{ctx}"
            )
            try:
...
async def cache_warm(req: WarmReq):
    ...
    for p in req.prompts:
        ...
        if not p:
            continue
        payload={"model":model,"messages":[{"role":"system","content":sys_prompt},{"role":"user","content":p}],
                 "temperature":TEMPERATURE,"max_tokens":256}
        result, _, _ = _run_direct(payload, force=None)
        if result:
            _ensure_semcache_import()
...
async def generate(request: Request, force: Optional[str] = None):
    ...
    # Override: domande meta/capability → mai web
...
async def web_search(req: WebSearchReq):
    ...
    if not ws.get("results"):
        return {"summary": (ws.get("summary") or ""), "results": [], "note": ws.get("note") or "non_web_query"}
    return {"summary": (ws.get("summary") or ""), "results": (ws.get("results") or [])}
