...
# + Semantic Cache (dual-write + similarity fallback + stats/ns) + endpoints admin + cache warm
...
# Semantic cache (con fallback su case dei path)
try:
    from core.semantic_cache import get_semantic_cache, SemanticCache
except Exception:
    # fallback legacy path/casing
    from Core.semantic_cache import get_semantic_cache, SemanticCache  # type: ignore
...
BUILD_SIGNATURE = "smart-intent-2025-11-07+pyspath-bootstrap+light-debug+semcache-v2.3-simstats+endpoints+warm"
...
_SEMCACHE: Optional[SemanticCache] = None
...
# ---- Similarity fallback (token-Jaccard 0..1) -----------------------
...
# ---- Semantic cache helper (dual-write con meta arricchito) ---------

import hashlib as _hashlib

def _semcache_dualwrite(prompt: str, system_prompt: str, model_name: str, used_intent: str, response_obj: Dict):
    """Salva la risposta in due namespace: intent-specific e AUTO (per pre-routing)."""
    try:
        if not _SEMCACHE: return
...
def healthz():
    ...
    # prova ad arricchire con dimensione/namespaces se disponibile
...
def endpoints_update(req: EndpointsUpdateReq, request: Request):
    # auth semplice via segreto in header o body
...
async def generate(request: Request, force: Optional[str] = None):
    ...
    try:
        if _SEMCACHE:
            ...
            if hit:
                ...
                _INTENT_FB.record_feedback(query=prompt, intent_used="CACHE_SEMANTIC", satisfaction=1.0, response_time_s=0.005)
                return out
    except Exception as e:
    ...
    # === Semantic cache (pre-LLM, dopo routing) ===
    try:
        if _SEMCACHE:
