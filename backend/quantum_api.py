...
async def chat(payload: dict = Body(...)) -> Dict[str, Any]:
    ...
    if isinstance(messages, list) and messages:
        ...
        # Ultimo messaggio user come testo principale
    ...
    if not sid:
        # fallback sicuro per richieste GUI/API senza source_id esplicito
        sid = "default"
    ...
    base_sys = user_sys_prompt or persona_store or "Sei una GPT neutra e modulare."
    if explicit_sys_from_messages:
        # I system messages espliciti nel payload hanno priorità logica,
        # ma non cancellano il persona: li concateno in testa
    ...
    if _SEMCACHE:
        try:
            ctx_fp = SemanticCache.fingerprint(base_sys, LLM_MODEL, "CHAT")
        ...
        if hit:
            ...
            # soglia alta: riusa solo quando è davvero la stessa domanda
            if sim >= 0.88:
    ...
    if _is_jarvis_hw_query(text):
        ...
        # Scrivi anche in semantic cache (così le richieste ripetute tornano istantanee)
        if _SEMCACHE:
            try:
                _semcache_dualwrite(
                    text,
...
def memory_search_advanced(req: SearchAdvancedReq) -> Dict[str, Any]:
    ...
    items = search_topk_with_filters(
        query=req.q, k=req.k, where=req.where, collections=cols
    )
    return {
        "ok": True,
        "q": req.q,
