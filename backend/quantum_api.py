...
#
# Patch 2025-11:
# - token budget hard-cap, /web/summarize (url|q), no-error to user, feedback toggle
...
# - Parallel fetching con timeout per-URL, early-exit, metriche fetch, timeout su /web_read
# - Web validator (consensus) + search analytics (p50/p90, CTR, cache-hit)
# - Guard relax: consentite query a 2 parole (es. “prezzo bitcoin”)
# - Warm hardening: niente 500, init cache garantita, errori raccolti
# - Explain-Guard: forza DIRECT_LLM per "spiega / che cos'è / what is" (no web)
# - Analytics endpoints: /analytics/search/report, /analytics/search/tail, /analytics/track_click
# - Mini-cache web: /web/cache/stats, /web/cache/flush (LRU+TTL+ETag/Last-Modified in core/web_tools)
# - WebResearchAgent: /web/research orchestratore multi-step (Claude-style)
# - Web summary direct: LLM sempre responsivo, niente "apri la fonte"
# - SearchDiversifier: diversificazione domini post-rerank
# - Synthesis aggressive: prompt robusti per _web_search_pipeline e /web/summarize
# - 2025-11-19: LLM Intent Classifier + stats endpoint
...
def _ensure_semcache_import() -> None:
    ...
    global _SCM_IMPORTED, get_semantic_cache, SemanticCache
    if _SCM_IMPORTED:
        return
    try:
...
BUILD_SIGNATURE = (
    ...
    "search-diversifier+synthesis-aggressive-v1+llm-intent"
)
...
def _postboost_ranked(
    ranked: List[Dict[str, Any]], prefer: List[str]
) -> List[Dict[str, Any]]:
    pref = set(prefer or [])
    for r in ranked:
...
def _semcache_dualwrite(
    ...
    try:
        ...
        ctx_fp_intent = SemanticCache.fingerprint(
            system_prompt, model_name, used_intent
        )
...
# ✅ Guard rilassata: non bloccare 2 parole, e mai bloccare live-query a 2 parole
...
async def _web_search_pipeline(
    ...
    # === ⚡ PARALLEL FETCHING con SMART BUDGET (NUOVO) ⚡ ===
    ...
    if topk and nsum > 0:
        ...
        # Top 1: 40%, Top 2: 30%, Top 3: 20%, Resto: 10%
        ...
        async def _fetch_one(
            item: Dict[str, Any], budget: int, index: int
        ) -> Optional[Dict[str, Any]]:
            """
            Fetch singolo con timeout e budget dinamico.

            Args:
                item: Risultato con url
                budget: Token budget per questo URL
                index: Posizione (0=top)
            """
            ...
            # Timeout dinamico: top results più tempo
            if index == 0:
                timeout = 8.0  # Top result: più tempo
            elif index <= 2:
                timeout = 6.0  # Top 3: tempo normale
            else:
                timeout = 4.0  # Resto: veloce

            try:
                text, og_img = await asyncio.wait_for(
                    fetch_and_extract(url, timeout=timeout),
                    timeout=timeout + 1.0,  # Timeout wrapper
                )

                if not text or len(text) < 50:
                    return None

                # Trim al budget assegnato
        ...
        # Launch parallel tasks (solo top nsum)
        tasks = [
        ...
        # Gather con early success detection
        # Se otteniamo 2+ risultati buoni entro 3s, possiamo fare early exit
        early_exit_time = 3.0
        early_exit_min = min(2, nsum)

        try:
            # Wait con timeout parziale per early exit
            ...
            # Check se abbiamo abbastanza risultati per early exit
            results_so_far = [
                t.result() for t in done
                if not t.exception() and t.result() is not None
            ]

            if len(results_so_far) >= early_exit_min and pending:
                # Abbiamo abbastanza, possiamo fare early exit
                log.info(
                ...
                for t in pending:
                    t.cancel()
                done_early = True
                extracts = results_so_far
            else:
                # Aspetta tutti
                if pending:
                    await asyncio.gather(*pending, return_exceptions=True)

                # Collect tutti i risultati
                for t in tasks:
                    try:
                        if not t.cancelled():
                            result = t.result()
            ...
            # Fallback: collect quello che abbiamo
            for t in tasks:
                try:
        ...
        # Mantieni ordine relevance originale
        extracts.sort(key=lambda x: x.get("index", 999))
    ...
    if _VALIDATOR and extracts:
        try:
            validation = _VALIDATOR.validate_consensus(
                q, extracts[: max(0, nsum)]
    ...
    if extracts:
        ...
        # Prompt ULTRA-AGGRESSIVO V2
        prompt = (
            "CONTESTO: Sei un assistente che sintetizza risultati di ricerca web.\n"
            "Devi SEMPRE fornire valore all'utente, mai rimandarlo alle fonti.\n"
            "\n"
            "ESTRATTI DAL WEB:\n"
            f"{ctx}\n"
            "\n"
            ...
            "=== REGOLE ASSOLUTE ===\n"
            "\n"
            "**VIETATO** (pena fallimento totale):\n"
            "- 'Le fonti non contengono abbastanza informazioni'\n"
            "- 'Ti consiglio di aprire/visitare/consultare le fonti'\n"
            "- 'Per maggiori dettagli consulta...'\n"
            "- Qualsiasi frase che rimanda l'utente altrove\n"
            "\n"
            "**OBBLIGATORIO**:\n"
            "1. Sintetizza TUTTO ciò che trovi negli estratti\n"
            "2. Usa forma discorsiva (3-6 frasi), NO elenchi puntati\n"
            "3. Includi SEMPRE numeri/date/dati concreti se presenti\n"
            "4. Se info parziali: 'Gli estratti coprono [X, Y] ma non specificano [Z]'\n"
            "5. Almeno 2-3 facts verificabili nella risposta\n"
            "\n"
            "**SE GLI ESTRATTI PARLANO D'ALTRO**:\n"
            "Dì chiaramente: 'Le fonti trattano [tema trovato] e menzionano [dettagli A, B], "
            "ma non parlano di [tema richiesto]. Per quello servirebbe cercare [suggerimento specifico]'\n"
            "\n"
            "**FORMATO**:\n"
            "- Italiano chiaro e diretto\n"
            "- 3-6 frasi (80-250 caratteri)\n"
            "- No citazioni esplicite tipo 'secondo X...'\n"
            "- Priorità ai dati live (prezzi, date, risultati)\n"
            "\n"
            "RISPONDI ORA:"
        )
        ...
        if summary:
            try:
                ...
                # Se qualità troppo bassa E abbiamo tempo, RETRY
...
def endpoints_update(req: EndpointsUpdateReq, request: Request) -> Dict[str, Any]:
    provided = (
        req.secret or request.headers.get("x-quantum-secret") or ""
    )
...
async def cache_warm(req: WarmReq) -> Dict[str, Any]:
    try:
        sys_prompt = (
            ...
            or "Sei una GPT neutra e modulare."
        )
        ...
        for p in (req.prompts or []):
            ...
            if result:
                try:
                    _semcache_dualwrite(
                        p, sys_prompt, model, "DIRECT_LLM", result
                    )
                    done += 1
                except Exception as e:
...
async def generate(
    ...
    try:
        if _SEMCACHE:
            _ensure_semcache_import()
            ctx_fp_auto = SemanticCache.fingerprint(
                system_prompt, model_name, "AUTO"
            )
    ...
    # Se per qualche motivo route è ancora None, crea un default
    if route is None:
        route = {
            "intent": used_intent,
            "confidence": 0.0,
    ...
    try:
        if _SEMCACHE:
            _ensure_semcache_import()
            ctx_fp = SemanticCache.fingerprint(
                system_prompt, model_name, used_intent
            )
...
async def web_summarize(payload: WebSummarizeQueryReq) -> Dict[str, Any]:
    # Caso: sintesi risultati di ricerca (q)
    if payload.q:
        ...
        ws = await _web_search_pipeline(
            ...
            nsum=int(payload.summarize_top or WEB_SUMMARIZE_TOP_DEFAULT),
        )
...
def analytics_track_click(req: ClickTrackReq) -> Dict[str, Any]:
    ...
    Non richiede SearchAnalytics.track_search: scrive direttamente
    una riga JSONL con type='click'.
    """
    try:
        path = getattr(_ANALYTICS, "path", SEARCH_ANALYTICS_LOG)
...
async def clear_intent_cache(secret: str = Body(...)) -> Dict[str, Any]:
    ...
    Richiede il QUANTUM_SHARED_SECRET.
    """
...
async def search_diversity_stats() -> Dict[str, Any]:
    ...
    Analizza gli ultimi N risultati dal file analytics.
    """
    result: Dict[str, Any] = {
        "diversifier": {
