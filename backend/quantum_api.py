...
async def _web_search_pipeline(q:str, src:str, sid:str, k:int, nsum:int)->Dict:
    ...
    # ====== Parallel fetching con timeout + early-exit ======
    ...
    while idx < len(candidates) and len(tasks) < inflight_limit:
        tasks.add(asyncio.create_task(_fetch_with_timeout(candidates[idx])))
        idx += 1
    ...
    while tasks:
        ...
        for d in done:
            ...
            if isinstance(res, dict) and "text" in res:
                extracts.append(res)
                if len(extracts) >= max(0, nsum):
                    for p in pending:
                        p.cancel()
                    await asyncio.gather(*pending, return_exceptions=True)
                    tasks = set()
                    break
            if idx < len(candidates) and len(pending) < inflight_limit and len(extracts) < max(0, nsum):
                task = asyncio.create_task(_fetch_with_timeout(candidates[idx]))
                pending.add(task)
                idx += 1
        tasks = pending
    ...
    if extracts:
        ...
        prompt = (
            "Leggi attentamente gli estratti dal web e rispondi DIRETTAMENTE alla domanda.\n"
            "- Rispondi sempre in modo chiaro, concreto e sintetico (3–6 frasi).\n"
            "- Usa SOLO informazioni che compaiono nei testi estratti: non aggiungere fatti nuovi.\n"
            "- Se trovi numeri (prezzi, temperature, percentuali, ecc.), inseriscili nella risposta specificando l'unità.\n"
            "- Se mancano dati chiave (es. numero esatto, data precisa), dillo chiaramente ma fornisci comunque "
            "il miglior riassunto possibile del contenuto delle fonti.\n"
            "- NON dire all'utente di 'aprire la fonte' per avere la risposta: la tua risposta deve essere autonoma.\n"
            "- Se le fonti sono discordanti, spiega brevemente le differenze.\n"
            "- Tono: professionale ma semplice, niente frasi vaghe o evasive.\n\n"
            f"DOMANDA: {q}\n\n"
            f"ESTRATTI DAL WEB:\n{ctx}"
        )
    ...
    try:
        if used_intent=="WEB_READ":
            ...
            if url:
                ...
                msg = ("Riassumi in punti chiave e proponi next steps (conciso).\n"
                       f"URL: {url}\n\nTESTO:\n{trimmed}")
                try:
                    summary = await reply_with_llm(msg, persona)
                except Exception:
                    summary = "Apri la fonte per il dettaglio aggiornato."
                try:
                    if summary:
        ...
        err={"ok":False,"error":"Richiesta troppo lunga o sorgente non disponibile. Apri la fonte principale per i dati aggiornati.",
...
async def web_summarize(payload: WebSummarizeQueryReq):
    ...
    try:
        ...
        text, og_img = "", None
    trimmed = trim_to_tokens(text or "", WEB_SUMMARY_BUDGET_TOK)
    prompt = ("Riassumi la pagina in punti chiave (conciso) e proponi eventuali prossimi passi.\n\n"
              f"URL: {url}\n\nTESTO:\n{trimmed}")
    try:
        summary = await reply_with_llm(prompt, persona)
    except Exception:
