#!/usr/bin/env python3
# backend/quantum_api.py — Smart routing + Chroma + Auto-Save robusto
...
from utils.chroma_handler import (
    ...
    _col, reembed_all                 # per /memory/list e /memory/reembed
)
...
def _light_debug_dump() -> Dict:
    """
    Evita di agganciare embedding_function. Usa il client nudo per listare e contare.
    """
    ...
    try:
        cli = chromadb.Client(_ChromaSettings(persist_directory=CHROMA_PERSIST_DIR, anonymized_telemetry=False))
        cols = []
        for c in cli.list_collections():
            try:
                # get_collection senza embedding_function
                col = cli.get_collection(name=c.name)
                try:
