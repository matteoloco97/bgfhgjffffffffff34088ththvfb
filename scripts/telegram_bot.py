#!/usr/bin/env python3
# scripts/telegram_bot.py — LLM-only chat + web manuale
# PATCH 2025-11-10:
# - Tutti i messaggi vanno a /chat (nessun fallback web automatico)
# - /web e /read usano /web/summarize su richiesta
# - Calculator locale (se disponibile)
# - Log puliti, lock single-instance, no “fonti” inline
...
async def on_startup(app):
    ...
    log.info("🌐 HTTP session pronta; /chat=%s | /web/summarize=%s", QUANTUM_CHAT_URL, QUANTUM_WEB_SUMMARY_URL)
...
async def call_web_read(url: str, http: aiohttp.ClientSession, chat_id: int) -> str:
    ...
    return (data.get("summary") or data.get("answer") or "Nessun contenuto estratto").strip()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• 🌐 Usa il web con /web <query> o /read <url>\n"
...
if __name__ == "__main__":
    ...
    log.info("🤖 Avvio Telegram Bot (LLM-only chat | web manuale) | polling")
