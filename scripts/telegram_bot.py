#!/usr/bin/env python3
# scripts/telegram_bot.py - Bot Telegram con AI Autonoma
...
import logging, os, sys, fcntl, re
...
async def call_web_search(query: str, http: aiohttp.ClientSession, chat_id: int) -> list[str]:
    ...
    if summary and not summary.startswith('{"error"') and len(summary) > 50:
        msgs.append(f"{summary}")
        
        results = data.get("results", [])
        if results and len(summary) < 200:
            lines = ["\n\n📎 _Fonti:_"]
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    log.info(f"🎯 Intent: {intent.value} (conf: {confidence:.2f}) - '{text[:50]}'")
    ...
    try:
        if intent == Intent.WEB_READ:
            ...
            await msg.reply_text(f"🌐 Cerco informazioni aggiornate...")
...
if __name__ == "__main__":
    ...
    log.info("🤖 Avvio Telegram Bot con AI Autonoma (mode=polling)")
