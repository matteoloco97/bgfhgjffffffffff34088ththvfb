#!/usr/bin/env python3
# core/smart_intent_classifier.py — Zero-Web-by-Default, Deterministic & Robust (v4.3-PATCHED)
# 🔧 PATCH:
...
# - Guard rilassata: niente blocco smalltalk per query a 2 parole (es. "prezzo bitcoin")
# - Live queries corte (meteo/prezzi/risultati/orari fino a 3 parole) → WEB_SEARCH
...
class SmartIntentClassifier:
    ...
    _META = re.compile(r"""(?ix)\b(
        chi\s+sei|cosa\s+puoi\s+fare|come\s+funziona|come\s+funzioni|limitazioni|capacita'|capabilities|
    ...
    _BIOGRAPHY = re.compile(r"""(?ix)\b(
        ...
        dimmi\s+chi\s+è|dimmi\s+chi\s+e'|tell\s+me\s+about
    )\b""")
    ...
    _RECENCY = re.compile(r"(?i)\b(oggi|adesso|ora|in\s+tempo\s+reale|live|ultim[ei]|breaking|aggiornat[oi]?)\b")
    _MARKETS = re.compile(r"(?i)\b(prezzo|quotazione|tasso|valore|cambio|borsa|azioni?|indice|btc|bitcoin|eth|ethereum|crypto|eur/?usd)\b")
    ...
    def _live_trigger_score(self, t: str, trace: List[str]) -> int:
        score = 0
        if self._RECENCY.search(t):
            score += 1
            trace.append("RECENCY")
        if self._MARKETS.search(t):
            score += 1
            trace.append("MARKETS")
        if self._WEATHER.search(t):
            score += 1
            trace.append("WEATHER")
        if self._SPORTS.search(t):
            score += 1
            trace.append("SPORTS")
        if self._SCHEDULE.search(t):
            score += 1
            trace.append("SCHEDULE")
        if t.strip().endswith("?"):
            score += 1
            trace.append("QUESTION_MARK")
        return score

    def _is_smalltalk_like(self, t: str) -> bool:
        """
        PATCH guard rilassata:
        - non considerare più "smalltalk" tutte le query a 2 parole
        - solo:
          * stringa vuota
          * 1 parola che è nel pattern SMALLTALK (ciao, hey, ok, perfetto...)
          * o match esplicito del pattern SMALLTALK
        """
        t = (t or "").strip()
        if not t:
            return True
        tokens = t.split()
        # una sola parola e coincide con un saluto/frase di rito
        if len(tokens) == 1 and self._SMALLTALK.search(t):
            return True
        # qualunque lunghezza ma che inizi chiaramente con smalltalk
    ...
    def _is_live_topic(self, tl: str) -> bool:
        """True se la query riguarda chiaramente mercati/meteo/sport/orari."""
        return (
            self._MARKETS.search(tl)
            or self._WEATHER.search(tl)
            or self._SPORTS.search(tl)
            or self._SCHEDULE.search(tl)
        ) is not None
    ...
    def classify(self, text: str) -> Dict[str, Any]:
        ...
        if self._is_smalltalk_like(tl):
            return {
                "intent": "DIRECT_LLM",
                "confidence": 0.95,
                "reason": "smalltalk_or_very_short",
        ...
        if urls:
            ...
            if has_http_or_www or self._EXPLICIT_READ.search(tl):
                return {
                    ...
                    "reason": "url_present",
                    "url": urls[0],
                    "analysis": {"trace": ["URL_PRESENT"], "urls": urls, "text": t},
                }
        ...
        if self._BIOGRAPHY.search(tl):
            if self._has_time_sensitive(tl):
                return {
                    "intent": "WEB_SEARCH",
                    "confidence": 0.88,
                    "reason": "bio_time_sensitive",
                    "analysis": {"trace": ["BIO", "TIME_SENSITIVE"], "text": t},
                }
            return {
                "intent": "DIRECT_LLM",
                "confidence": 0.92,
                "reason": "biographical_knowledge",
        ...
        if self._EVERGREEN.search(tl):
            if self._has_time_sensitive(tl):
                return {
                    "intent": "WEB_SEARCH",
                    "confidence": 0.88,
                    "reason": "evergreen_but_time_sensitive",
                    "analysis": {"trace": ["EVERGREEN", "TIME_SENSITIVE"], "text": t},
                }
            return {
                "intent": "DIRECT_LLM",
                "confidence": 0.9,
                "reason": "evergreen_definition",
        ...
        # 8) trigger "live":
        #    - se score >=2 → WEB_SEARCH
        #    - oppure score >=1 + topic live + query corta (<=3 parole) → WEB_SEARCH
        score = self._live_trigger_score(tl, trace)
        tokens = t.split()
        is_live_topic = self._is_live_topic(tl)

        if score >= 2 or (score >= 1 and is_live_topic and 1 <= len(tokens) <= 3):
            return {
                "intent": "WEB_SEARCH",
                "confidence": 0.9,
                "reason": f"live_triggers[{score}]",
